...
class ConfirmExpiry(models.TransientModel):
    ...
    lot_ids = fields.Many2many(
        'stock.production.lot', readonly=True, required=True)
    picking_ids = fields.Many2many('stock.picking', readonly=True)
    description = fields.Char(
        'Description', compute='_compute_descriptive_fields')
    show_lots = fields.Boolean(
        'Show Lots', compute='_compute_descriptive_fields')
    ...
    def process(self):
        picking_to_validate = self.env.context.get(
